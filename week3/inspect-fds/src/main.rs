...
fn main() {
    ...
    let process_target: Option<Process> =  get_target(target).expect("get target failed");
    match process_target {
        Some(process) => {
            println!("Running `{}`", process.command);
        }
        None => {
            println!("Could not find target process {}", target);
            std::process::exit(1);
        }
    }
}
